...
import { UPDATE_CLIENT } from 'mutatuins/client'

const EditClientModal = ({ isActive, initData, onClose }) => {
    ...
    updateClient({
      ...
      update: (cache, { data: { updateClient } }) => {
        ...
        cache.writeQuery({
          query: GET_CLIENTS,
          data: {
            clients: clients.map(client => (client._id === variables.id ? { ...client, ...updateClient } : client))
          }
        })
      }
    })
    ...
    <>
      <div className='fixed inset-0 flex items-center justify-center z-30'>
        <div className='relative bg-gray-900 rounded-lg p-6 shadow-xl z-50'>
          <h2 className='text-2xl font-bold mb-5'>Edit Client</h2>
